...
class AI:
    ...
    def set_search(self):
        ...
        if self.type == "dfs":
            ...
            self.F_value = abs(self.grid.start[0] - self.grid.goal[0]) + abs(self.grid.start[1] - self.grid.goal[1])
            self.frontier = [(self.F_value, self.grid.start)]
            heapify(self.frontier)
            self.temp = []
            self.checked = []
            self.explored = []
